...
namespace Common.Validation
{
    public class FlightValidator : AbstractValidator<FlightDto>
    {
        public FlightValidator()
        {
            ...
            RuleFor(f => f.DepartureAirport).NotEmpty().Matches(@"[A-Z]{3}").MinimumLength(3).MaximumLength(3);
            RuleFor(f => f.DepartureTime).NotEmpty();
            RuleFor(f => f.DestinationAirport).NotEmpty().Matches(@"[A-Z]{3}").MinimumLength(3).MaximumLength(3);
